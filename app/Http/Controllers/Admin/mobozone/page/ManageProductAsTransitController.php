...
class ManageProductAsTransitController extends Controller
{
    public function transit(){
        ...
        foreach($order as $orders)
        {
            ...
            if($data2->data->order_status == "In_Transit"){
                // $lastdata[] = $orders_id;

                $getorder = Order::find($orders_id);

                $getorder->order_status2 = "In_Transit";
                $getorder->save();
            }
        ...
        // dd($access_token);
        $order = DB::table('orders')->where('order_status2', "In_Transit")->where('is_fake',0)->get();
        $totalorder = DB::table('orders')->where('order_status2', "In_Transit")->where('is_fake',0)->count();
        ...
        return view('admin.mobozone.pages.manageorder.transinorder', $data);
