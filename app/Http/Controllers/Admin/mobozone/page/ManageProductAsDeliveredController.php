...
class ManageProductAsDeliveredController extends Controller
{
    public function delevery(){
        ...
        foreach($order as $orders)
        {
            ...
            if($data2->data->order_status == "Delivered"){
                // $lastdata[] = $orders_id;

                $getorder = Order::find($orders_id);

                $getorder->order_status2 = "Delivered";
                $getorder->save();
            }
        ...
        // dd($access_token);
        $order = DB::table('orders')->where('order_status', "Delivered")->where('is_fake',0)->get();
        $totalorder = DB::table('orders')->where('order_status', "Delivered")->where('is_fake',0)->count();
        ...
        return view('admin.mobozone.pages.manageorder.delivedorder', $data);
